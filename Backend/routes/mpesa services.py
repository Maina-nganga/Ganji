...
def pay_till(phone_number, amount, till_number, account_ref="Payment"):
...
def pay_paybill(phone_number, amount, paybill_number, account_number):
    """STK push to a Paybill number."""
    token = get_access_token()
    password, timestamp = generate_password()
    shortcode = os.getenv("MPESA_SHORTCODE")
    callback_url = os.getenv("MPESA_CALLBACK_URL")
